...
import CreateMissingAlertButton from "./components/CreateMissingAlertButton";
import MissingAnimalListingHeading from "./components/MissingHeading";
import usePaginatedMissingAlertsQuery from "./usePaginatedMissingAlertsQuery";

export default function MissingAnimalListings() {
  ...
  const { paginatedAlerts, isError, isLoading, isFetchingNextPage, missingAlerts, setQueryData } =
    usePaginatedMissingAlertsQuery(pageSize, filtersQueryString);
  ...
  return (
    <>
      <main className="relative">
        <MetaTags
          title="Animais Desaparecidos | AcheMeuPet"
          description="Veja animais que estão desaparecidos e ajude a encontrá-los. Ajude a reunir pets perdidos com seus donos."
          keywords="animais desaparecidos, pets perdidos, busca de animais, animais sumidos, pet desaparecido"
        />
        <Header />
        <section className="lg:flex mb-6">
          {isMobile ? (
            <div className="flex max-[360px]:flex-col max-[360px]:gap-2.5 justify-between mt-4 mx-6">
              ...
              />
              {showOutsideNewAlertBtn && <CreateMissingAlertButton />}
            </div>
          ) : (
            <AlertFilters
              isMobile={false}
