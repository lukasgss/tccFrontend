  ...
  const filteredAlerts = userCreatedAlerts
    ? [
        ...userCreatedAlerts.adoptionAlerts
          .filter((alert) => alert.pet.name?.toLowerCase().startsWith(search.toLowerCase()))
          .map((alert) => ({ ...alert, type: "adoption" as const })),
        ...userCreatedAlerts.foundAnimalAlerts
          .filter((alert) => alert.name?.toLowerCase().startsWith(search.toLowerCase()))
    ...
    <main className="relative">
      ...
      >
        <div className="flex flex-col gap-4 lg:flex-row justify-between">
          ...
          {userCreatedAlerts &&
          userCreatedAlerts.adoptionAlerts.length > 0 &&
          userCreatedAlerts.foundAnimalAlerts.length > 0 ? (
            <TextInput
              radius="xl"
              size="md"
