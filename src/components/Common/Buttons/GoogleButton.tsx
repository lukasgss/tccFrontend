...
export default function GoogleButton({ setErrorMessage }: GoogleButtonProps) {
  ...
  async function googleAuthenticate(googleData: CodeResponse) {
    if (!googleData.code) {
      return;
    }
    await mutation.mutateAsync({ provider: "GOOGLE", idToken: googleData.code });
  }
